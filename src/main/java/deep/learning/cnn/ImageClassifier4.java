...
public class ImageClassifier4 {
    protected static final Logger log = LoggerFactory.getLogger(ImageClassifier4.class);
...
//	protected static int width = 240;
    ...
    public void run(String[] args) throws Exception {
        ...
        File mainPath = new File("datafashion\\images-resize\\Number\\train");
...
//        MultiLayerNetwork network = new AlexNet(height, width, channels, numLabels, seed, 1000).init();
...
//        network.fit(dataIter,epochs);
        ...
        mainPath = new File("datafashion\\images-resize\\Number\\test");
        fileSplit = new FileSplit(mainPath, NativeImageLoader.ALLOWED_FORMATS, rng);
        numExamples = toIntExact(fileSplit.length());
        numLabels = fileSplit.getRootDir().listFiles(File::isDirectory).length; //This only works if your root is clean: only label subdirs.
    	pathFilter = new RandomPathFilter(rng, NativeImageLoader.ALLOWED_FORMATS);
		input = fileSplit.sample(pathFilter);
		InputSplit testData = input[0];
        ...
        if (save) {
            log.info("Save model....");
            ModelSerializer.writeModel(network, "datafashion\\models\\model-alexnet-number-240-320.bin", true);
        }
